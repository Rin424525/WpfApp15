...
namespace WpfApp15.Model
{
    public class SqlModel
    {
        ...
        internal List<Special> SelectSpecialsByGroup(Special selectedSpecial)
        {
            int id = selectedSpecial?.ID ?? 0;
        ...
        internal List<curator> SelectCuratorsByGroup(Group selectedGroup)
        {
            int id = selectedGroup?.ID ?? 0;
        ...
        internal List<discipline> SelectDisciplinesByPrepods(Prepods selectedPrepods)
        {
            int id = selectedPrepods?.ID ?? 0;
            var discipline = new List<discipline>();
            var mySqlDB = MySqlDB.GetDB();
            string query = selectedPrepods != null ? 
                $"SELECT * FROM `discipline` where prepod_id = "+selectedPrepods.ID
                :
        ...
        internal List<journal> JournalBySelectedStudentDiscipline(Student SelectedStudent, discipline SelectedDiscipline)
        {
            int id = SelectedStudent?.ID ?? 0;
            var students = new List<journal>();
            var mySqlDB = MySqlDB.GetDB();
            string query = $"SELECT * FROM `journal`";
            if (SelectedStudent != null && SelectedDiscipline != null)
                query += $" where discipline_id = {SelectedDiscipline.ID} and student_id = {SelectedStudent.ID}";
            else if (SelectedStudent == null && SelectedDiscipline != null)
                query += $" where discipline_id = {SelectedDiscipline.ID}";
            else if (SelectedStudent != null && SelectedDiscipline == null)
                ...
                using (MySqlDataReader dr = mc.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        groups.Add(new Group
                        {
                            ID = dr.GetInt32("id"),
                            Title = dr.IsDBNull(1) ? "" : dr.GetString("title"),
                            Year = dr.IsDBNull(1) ? 0 : dr.GetInt32("year")
                        });
                    }
                }
                mySqlDB.CloseConnection();
        ...
        public List<Special> SelectSpecialsRange(int skip, int count)
        {
            var specials = new List<Special>();
            var mySqlDB = MySqlDB.GetDB();
            string query = $"SELECT * FROM `specials`";
        ...
        public List<Student> SelectStudentsRange(int skip, int count)
        {
            var students = new List<Student>();
            var mySqlDB = MySqlDB.GetDB();
        ...
        public List<Prepods> SelectPrepodsRange(int skip, int count)
        {
        ...
        public List<discipline> SelectDisciplineRange(int skip, int count)
        {
            var Discipline = new List<discipline>();
            var mySqlDB = MySqlDB.GetDB();
